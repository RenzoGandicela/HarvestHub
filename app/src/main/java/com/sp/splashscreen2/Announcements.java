...
public class Announcements extends AppCompatActivity {
   ...
   private void submitNewEvent() {
        ...
        if (eventFormLayout.getVisibility() == View.VISIBLE) { // Type 1 form is visible
            ...
            if (eventId != null) {
                eventsRef.child(eventId).setValue(event).addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        ...
                        toggleEventForm(false, eventType);  //HERE
                        clearForm();
            ...
            String eventId = donationRef.push().getKey();
            Map<String, Object> event = new HashMap<>();
            event.put("title", title);
            event.put("description", description);
            event.put("details", details);
            ...
            if (eventId != null) {
                donationRef.child(eventId).setValue(event).addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        Toast.makeText(this, "Event added successfully!", Toast.LENGTH_SHORT).show();
                        toggleEventForm(false, eventType);  //HERE
                        clearForm();
                        // Update the otherEventList
                        otherEventList.add(new FeaturedHelperClass(image, title, description, location, details, eventType));
                        otherAdapter.notifyDataSetChanged();

                    } else {
                        Toast.makeText(this, "Failed to add gathering event.", Toast.LENGTH_SHORT).show();
                    }
                });
            }
        } else {
    ...
    // Added for type 2 event submission
    ...
    private void setupRecyclerView() {
        ...
        featuredAdapter = new FeaturedAdapter(this, eventList);
        featuredRecycler.setAdapter(featuredAdapter);
    }

    // Added setup for second recycler view (type 2 events)
    private void setupRecyclerViewType2() {
        featuredRecyclerType2.setHasFixedSize(true);
        featuredRecyclerType2.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false));

        otherAdapter = new OtherAdapter(this, otherEventList);
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot eventSnapshot : snapshot.getChildren()) {
                    ...
                    if (eventType != null && eventType == 1) {
                        ...
                        // If eventType is null, handle it as needed (e.g., put in otherEventList)
                        String defaultEventType = "Unknown"; // or any default value
                        otherEventList.add(new FeaturedHelperClass(image, title, description, location, details, 0));
                    }
                }

                featuredAdapter.notifyDataSetChanged();
                otherAdapter.notifyDataSetChanged(); // Update second RecyclerView
            }

            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(Announcements.this, "Failed to load events.", Toast.LENGTH_SHORT).show();
            }
        });

        // Listen for updates in Donation Events
        donationRef.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                otherEventList.clear();  // Clear the second list
                ...
                otherAdapter.notifyDataSetChanged();  // Update second RecyclerView (Gathering Events)
            }

            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(Announcements.this, "Failed to load gathering events.", Toast.LENGTH_SHORT).show();
            }
        });
    }
}
